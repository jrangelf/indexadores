...
from bt_data_tools import verificar_data_atualizacao, formatar_data_ymd
import datetime
...
def selecionar_tabelas(query):
    with conectar_sql() as conn:
        if conn:
            ...
            #print(f"rows: {rows}")
            #print(f"retorna: {[row[0] for row in rows]}")
    ...
    with conectar_sql() as conn:
        if conn is not None:
            ...
            for codigo, nome, data in lista:
                data_formatada = formatar_data_ymd(data) #data.strftime("%Y-%m-%d")
                try:
                    query_substituida = query.replace('$1', data_formatada).replace('$2', str(codigo))            
                    cursor.execute(query_substituida)
                    print(f"{nome} : {data_formatada}")
                    conn.commit
                except psycopg2.Error as e:
                    print(f"Erro ao executar query para {nome}: {e}")
            print("=============================================")                        
    return None
...
def update_processar_logatualizacao(lista,data_atual,query):
    ...
    with conectar_sql() as conn:
        if conn is not None:
            cursor = conn.cursor()
            print("===============================================================")
            print("processar (logatualizacao) atualizada")        
            for codigo, data_log in lista:                
                if verificar_data_atualizacao(codigo, data_atual, data_log):
                    query_substituida = query.replace('$', str(codigo))                
                    cursor.execute(query_substituida)
                    print(f"codigo: {codigo} data_log: {data_log} processar=1")                    
            conn.commit()     
